...
const verifyPhone = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      const body = req.body;
      body.user_phone = body.user_phone.toString().trim();
      ...
      if (!user) {
        ...
        let userResponse = await UserModel.create({
          user_phone: commonFunction.trim(
            commonFunction.toLowerCase(body.user_phone)
          ),
          user_id: user_id,
          user_verify_otp: otp,
        }).catch((e) =>
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let user = await UserModel.findOneAndUpdate(
        ...
        {
          $set: {
            user_name: commonFunction.trim(
              commonFunction.toLowerCase(body.user_name)
            ),
            user_email: commonFunction.trim(
              commonFunction.toLowerCase(body.user_email)
            ),
          },
        },
        {
...
const signUp = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let userResponse = await UserModel.create({
        user_fname: commonFunction.trim(
          commonFunction.toLowerCase(body.user_fname)
        ),
        user_lname: commonFunction.trim(
          commonFunction.toLowerCase(body.user_lname)
        ),
        user_email: commonFunction.trim(
          commonFunction.toLowerCase(body.user_email)
        ),
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve({
        ...
        count:
          response.count &&
          response.count.lenght &&
          response.count[0].totalCount
            ? response.count[0].totalCount
            : 0,
      });
    } catch (e) {
